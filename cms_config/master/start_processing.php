 ...
 */
$loginCheck = 0;
if (isset($_SESSION['master_login']['status'])) {
	$loginCheck = $_SESSION['master_login']['status'];
}
...
if ($loginCheck == 1) {
	...
	if ($currentAccount) {
		#運営アカウント以外であればセッションを破棄してログイン画面へ
		if ($currentAccount['account_type'] !== 'operator' || $currentAccount['shop_id'] !== null) {
			$_SESSION = [];
			session_destroy();
			header("Location: ./");
			exit;
