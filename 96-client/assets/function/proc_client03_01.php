<?php
/*
 * [96-client/assets/function/proc_client03_01.php]
 *  - 加盟店管理画面 -
 *  受注一覧：検索/ページング
 *
 * [初版]
...
if ($shopId === null || ctype_digit((string)$shopId) === false || (int)$shopId < 1) {
	header('Content-Type: application/json; charset=UTF-8');
	$makeTag['status'] = 'error';
	$makeTag['title'] = 'セッションエラー';
...
if ($action === 'updateStatus') {
	...
	$orderData = getShopOrderForStatusUpdate((int)$orderIdRaw, $shopId);
...
HTML;
			...
			#返品ステータスは変更不可
			$pointerEvents = '';
			if ((int)$targetOrderStatusId === 9) {
				$pointerEvents = 'style="pointer-events:none;"';
			}
			$makeTag['tag'] .= <<<HTML
                    <button type="button" class="selectbox__head" aria-expanded="false" {$pointerEvents}>
