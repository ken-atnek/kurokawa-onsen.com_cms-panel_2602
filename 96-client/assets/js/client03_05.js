 ...
 *
 */
const requestURL = "./assets/function/proc_client03_05.php";

/**
 * 規格並び替え（↑↓／D&D）
 *
 */
function getSpecificationListRoot() {
    return document.querySelector(".drag-area");
}
function getSpecificationItemElements() {
    const ul = getSpecificationListRoot();
    if (!ul) return [];
...
function getSpecificationIdFromItem(li) {
...
function getCurrentOrderIds() {
    return getSpecificationItemElements()
        .map((li) => getSpecificationIdFromItem(li))
        .filter((id) => Number.isInteger(id) && id > 0);
}
function updateSortButtonVisibility() {
    const items = getSpecificationItemElements();
...
function restoreOrderByIds(orderIds) {
    ...
    const map = new Map();
    items.forEach((li) => {
        const id = getSpecificationIdFromItem(li);
        if (id) map.set(id, li);
    });
    //既存規格行を一旦除去（DOMから消すだけ）
...
function getDragAfterElement(container, y) {
    const sortableLis = getSpecificationItemElements().filter((li) => !li.classList.contains("is-dragging"));
        ...
        try {
            ...
            if (!blockModal) {
                if (list["status"] == "error") {
                    ...
                    window.location.href = "client03_05.php";
                }
                return;
            }
            //サーバー応答がエラーの場合
            if (list["status"] == "error") {
                ...
                //一覧へ戻るボタン生成
                let newButton = `<button type="button" class="btn-cancel" onclick="closeModalToPage('client03_05.php');">閉じる</button>`;
                blockModal.querySelector(".box-btn").insertAdjacentHTML("beforeend", newButton);
                //「✕」ボタンも変更
                blockModal.querySelector(".box-title").querySelector("button").setAttribute("onclick", "closeModalToPage('client03_05.php')");
            }
            blockModal.classList.add("is-active");
            document.documentElement.style.overflow = "hidden";
        } catch (error) {
 ...
 *
 */
function deleteSpecification(el) {
    const listItem = el?.closest("li");
    if (!listItem) return;
    //規格ID（一覧の表示IDをそのまま利用）
    const specificationId = (listItem.querySelector(".item-id span")?.textContent || "").trim();
    if (!specificationId) return;
    //規格名（表示用）
    const specificationName = (listItem.querySelector(".item-name span")?.textContent || "").trim();
    //セッションキー
    const noUpDateKey = document.querySelector('form[name="inputForm"] input[name="noUpDateKey"]')?.value || "";
    ...
    const blockModal = document.getElementById("modalBlock");
    if (!blockModal) {
        const label = specificationName ? `「${specificationName}」` : "この規格";
        if (window.confirm(`${label}を削除します。よろしいですか？`)) {
            confirmDeleteSpecification(specificationId, noUpDateKey);
        }
        return;
    }
    //削除対象をモーダルに保持
    blockModal.dataset.specificationId = specificationId;
    ...
    if (titleEl) titleEl.textContent = "規格削除";
    if (msgEl) {
        const label = specificationName ? `「${specificationName}」` : "この規格";
    ...
    const buttons = `
        ...
        <button type="button" class="btn-confirm" onclick="confirmDeleteSpecification();">はい</button>
    `;
 ...
 *
 */
async function confirmDeleteSpecification(forcedSpecificationId, forcedNoUpDateKey) {
    const blockModal = document.getElementById("modalBlock");
    const specificationId = forcedSpecificationId || blockModal?.dataset.specificationId || "";
    const noUpDateKey = forcedNoUpDateKey || blockModal?.dataset.noUpDateKey || "";
    if (!specificationId || !noUpDateKey) return;
    const sFd = new FormData();
    sFd.append("action", "deleteSpecification");
    sFd.append("method", "delete");
    sFd.append("specificationId", specificationId);
    sFd.append("noUpDateKey", noUpDateKey);
    try {
        const response = await fetch(requestURL, {
            method: "POST",
        ...
        if (result["status"] == "error") {
            ...
            blockModal.querySelector(".box-title p").innerHTML = result["title"] || "規格削除";
            blockModal.querySelector(".box-details p").innerHTML = String(result["msg"] || "削除が完了しました。").replace(/\n/g, "<br>");
            const btnBox = blockModal.querySelector(".box-btn");
            btnBox.querySelectorAll("button").forEach((b) => b.remove());
            btnBox.insertAdjacentHTML("beforeend", `<button type="button" class="btn-cancel" onclick="closeModalToPage('client03_05.php');">閉じる</button>`);
            blockModal.querySelector(".box-title")?.querySelector("button")?.setAttribute("onclick", "closeModalToPage('client03_05.php')");
        }
        blockModal.classList.add("is-active");
        document.documentElement.style.overflow = "hidden";
 ...
 *
 */
async function editSpecification(el) {
    const listItem = el?.closest("li");
    if (!listItem) return;
    //規格ID（一覧の表示IDをそのまま利用）
    const specificationId = (listItem.querySelector(".item-id span")?.textContent || "").trim();
    ...
    try {
        ...
        if (!blockModal && list["status"] == "error") {
            showErrorModal(list["title"] || "規格情報", list["msg"] || "規格情報の取得に失敗しました。");
            return;
        }
        //サーバー応答がエラーの場合
        if (list["status"] == "error") {
            const title = list["title"] || "規格情報";
            const msgRaw = list["msg"] || "規格情報の取得に失敗しました。\nお手数ですが最初からやり直してください。";
            const msg = String(msgRaw).replace(/\n/g, "<br>");
            blockModal.querySelector(".box-title p").innerHTML = title;
            blockModal.querySelector(".box-details p").innerHTML = msg;
            //ボタン再生成
...
window.confirmDeleteSpecification = confirmDeleteSpecification;
//初期化
(() => {
    const init = () => {
        initSortButtons();
