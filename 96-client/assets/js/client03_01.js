...
function resetOrderSearchForm(searchForm) {
    ...
    const hiddenStatus = searchForm.querySelector('input[name="searchStatus"][type="hidden"]');
    if (hiddenStatus) hiddenStatus.value = "";
    searchForm.querySelectorAll('input[name="searchStatus"][type="radio"]').forEach((radio) => {
        radio.checked = false;
    });
    const statusSelectBox = searchForm.querySelector(".select-search-status[data-selectbox]");
    const statusValue = statusSelectBox?.querySelector("[data-selectbox-value]");
    if (statusValue) statusValue.textContent = "選択してください";
    if (statusSelectBox) {
        statusSelectBox.classList.remove("is-selected");
        statusSelectBox.classList.add("is-empty");
    }
}
