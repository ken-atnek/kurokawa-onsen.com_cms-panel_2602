        ...
        <div class="block_inner">
          ...
          <article class="inner_search-list">
            ...
            <form name="inputForm" class="inputForm box-new-entry">
              ...
              <div>
                <span>規格名</span>
                <input type="text" name="newSpecificationName" class="required-item" placeholder="規格名を入力" required maxlength="255">
              </div>
              <div>
                <span>管理名</span>
                <input type="text" name="newAdminName" placeholder="省略可" maxlength="255">
              </div>
            ...
            <ul class="drag-area">
              <li>
                <div></div>
                <div>ID</div>
                <div style="align-items: flex-start">規格管理</div>
                <div></div>
              </li>

HTML;
#表示可能リストあればループで差し込む
if (!empty($itemSpecificationsList)) {
  $totalSpecificationCount = count($itemSpecificationsList);
  $specificationIndex = 0;
  foreach ($itemSpecificationsList as $itemSpecification) {
    $itemSpecificationID = htmlspecialchars($itemSpecification['specification_id'], ENT_QUOTES, 'UTF-8');
    $itemSpecificationName = htmlspecialchars($itemSpecification['name'], ENT_QUOTES, 'UTF-8');
    $itemSpecificationAdminName = htmlspecialchars($itemSpecification['backend_name'], ENT_QUOTES, 'UTF-8');
    $itemSpecificationCount = htmlspecialchars((string)($itemSpecification['class_category_count'] ?? 0), ENT_QUOTES, 'UTF-8');
    $isFirst = ($specificationIndex === 0);
    $isLast = ($specificationIndex === ($totalSpecificationCount - 1));
    ...
    print <<<HTML
              <li>
                ...
                <div class="item-id">
                  <span>{$itemSpecificationID}</span>
                </div>
                <div class="item-name">
                  <a href="./client03_05_01.php?specification_id={$itemSpecificationID}" class="name">{$itemSpecificationName}</a>
                  <span class="admin">{$itemSpecificationAdminName}</span>
                  <span class="number">{$itemSpecificationCount}</span>
                </div>
                <nav>
                  ...
                  <button class="btn-edit" type="button" data-tooltip="編集" aria-label="編集" onclick="editSpecification(this)"></button>
                  <button class="btn-delate" type="button" data-tooltip="削除" aria-label="削除" onclick="deleteSpecification(this)"></button>
                </nav>
              </li>
...
print <<<HTML
    ...
    <script src="./assets/js/client03_05.js" defer></script>
  </body>
</html>
